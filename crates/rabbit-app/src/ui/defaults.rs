...
pub fn ping_options() -> String {
    ...
    let count = if let Ok(c) = load_config() {
        c.modules.get_integer("ping", "count").unwrap_or(-1)
    } else { -1 };
...
pub fn scan_options() -> String {
    let filter = if let Ok(c) = load_config() {
        c.modules.get_bool("scan", "filter").unwrap_or(true)
    } else { true };
...
pub fn tftpd_options() -> String {
    ...
    format!("timeout={};retry={};blksize={};override={};qsize={};qtout={};fslog={};",

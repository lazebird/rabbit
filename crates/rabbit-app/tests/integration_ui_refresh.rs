...
use rabbit_core::ui_channel::{Module, UiData};
use rabbit_models::config::{AppConfig, ConfigValue};
use tokio::sync::mpsc;

/// 辅助：创建带 channel 的 PingService 并模拟启动
async fn setup_ping_with_target(target: &str) -> (rabbit_core::PingService, mpsc::Receiver<UiData>) {
    ...
    let mut svc = rabbit_core::PingService::with_channel(tx);
...
async fn scan_complete_should_send_service_status_false() {
    ...
    let mut svc = rabbit_core::ScanService::with_channel(tx);
...
async fn ui_refresh_scan_running_should_reflect_actual_state() {
    // 初始化全局状态
    let _state = rabbit_app::UiState::init();

    // 设置 scan_running=true
    rabbit_app::set_scan_running(true);

    // 验证可以通过全局状态获取
    if let Some(state) = rabbit_app::UiState::global() {
    ...
    rabbit_app::set_scan_running(false);
...
fn service_update_result_is_running_should_not_exist() {
    ...
    let result = rabbit_core::ServiceUpdateResult::Started("test".to_string());
